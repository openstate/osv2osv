...
pub fn data2eml (data: OsvInternalData) -> String {
...
pub fn csv2data(r: Box<Read>) -> OsvInternalData {
    ...
    data.election_date = {
        let datum = get_field!(iter, "Datum");
        if datum.len() > 3 && &datum[2..3] == "-" { // somebody reversed RFC3339 / ISO8601 order :(
            let d = NaiveDate::parse_from_str(&datum, "%d-%m-%Y").expect("Datum is too corrupt");
    ...
    for _i in 0..4-data.gm_code.len() {
        data.gm_code.insert_str(0, "0"); // Pad with zeros because of possible xlsx removal
    }
    ...
    if data.election_identifier_name.len() > 19 && &data.election_identifier_name[..19] == "Provinciale Staten " {
        ...
        let offset = {
            if data.election_identifier_name.len() > 36 && &data.election_identifier_name[25..36] == "waterschap " {
                36
            } else if data.election_identifier_name.len() > 43 && &data.election_identifier_name[25..43] == "hoogheemraadschap " {
                43
            } else {
